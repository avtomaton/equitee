...
import { sn, WindowPicker, CHART_TOOLTIP_STYLE } from './uiHelpers';
import { COLORS } from '../config';
import { avgMonthly, yearsHeld, monthsLeftInYear } from '../metrics';
import type { Property } from '../types';
import { usePortfolioAggregates } from '../hooks/usePortfolioAggregates';
import usePortfolioMetrics from '../hooks/usePortfolioMetrics';
import { cardAvgIncome, cardAvgExpenses, cardAvgCashFlow, cardAvgNOI, cardCapRate, cardOER, cardDSCR, cardICR, cardMonthlyGain, cardNetPosition, cardPaybackPeriod, cardBreakEven, cardTotalAppreciation, cardYearlyAppreciation, cardProjectedYearEnd, cardYearEndBalance, cardAvailEquityPortfolio, cardMortgagePerMonth, cardYtdOpProfit } from '../metricDefs';
import type { Income, Expense, Event } from '../types';

const STATUS_COLORS: Record<string, string> = { Rented: '#10b981', Vacant: '#ef4444', Primary: '#3b82f6' };
const G2: CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', marginBottom: '1.25rem' };

interface ChartData {
  incExp: Array<{ name: string; Income: number; Expenses: number }>;
  value: Array<{ name: string; 'Market Value': number; 'Purchase Price': number }>;
  roi: Array<{ name: string; ROI: number }>;
  status: Array<{ name: string; value: number }>;
  equity: Array<{ name: string; Equity: number; Loan: number }>;
  equityPct: Array<{ name: string; EquityPct: number }>;
  appreciation: Array<{ name: string; Appreciation: number; YearlyAppr: number | null }>;
  capRates: Array<{ name: string; 'Cap Rate': number }>;
  ltv: Array<{ name: string; LTV: number }>;
}

/** Build static chart datasets from property records (no async data needed). */
function buildChartData(list: Property[]): ChartData {
  ...
  const value = list.map(p => ({
    name: sn(p.name),
    'Market Value': p.market_price ?? 0,
    'Purchase Price': p.purchase_price ?? 0,
  }));
  const roi = list.map(p => {
    const net = (p.total_income ?? 0) - ((p.total_expenses ?? 0) - ((p.purchase_price ?? 0) - (p.loan_amount ?? 0)));
    return { name: sn(p.name), ROI: (p.market_price ?? 0) > 0 ? parseFloat((net / p.market_price! * 100).toFixed(2)) : 0 };
  });
  const statusCount: Record<string, number> = list.reduce((acc: Record<string, number>, p) => { const status = p.status ?? 'Unknown'; acc[status] = (acc[status] || 0) + 1; return acc; }, {});
  const status = Object.entries(statusCount).map(([name, value]) => ({ name, value }));
  const equity = list.map(p => ({ name: sn(p.name), Equity: (p.market_price ?? 0) - (p.loan_amount ?? 0), Loan: p.loan_amount ?? 0 }));
  const equityPct = list.map(p => ({
    name: sn(p.name),
    EquityPct: (p.market_price ?? 0) > 0 ? parseFloat((((p.market_price ?? 0) - (p.loan_amount ?? 0)) / p.market_price! * 100).toFixed(1)) : 0,
  }));
  ...
  const capRates = list
    .filter(p => (p.purchase_price ?? 0) > 0 && (p.monthly_rent ?? 0) > 0)
    .map(p => ({
      name: sn(p.name),
      'Cap Rate': parseFloat((((p.monthly_rent ?? 0) * 12) / p.purchase_price! * 100).toFixed(2)),
    }))
    .sort((a, b) => b['Cap Rate'] - a['Cap Rate']);
  const ltv = list
    .filter(p => (p.loan_amount ?? 0) > 0 && (p.market_price ?? 0) > 0)
    .map(p => ({
      name: sn(p.name),
      LTV: parseFloat(((p.loan_amount ?? 0) / p.market_price! * 100).toFixed(1)),
    }))
...
function buildCashFlowTrend(allIncome: Income[], allExpenses: Expense[], months = 12): Array<CashFlowBucket & { 'Cash Flow': number }> {
  const now = new Date();
  const buckets: Record<string, CashFlowBucket> = {};
  ...
  allIncome.forEach(r => {
    ...
    if (key && buckets[key]) buckets[key].Income += r.amount ?? 0;
  });
  allExpenses.forEach(r => {
    const key = r.expense_date?.slice(0, 7);
    if (key && buckets[key]) buckets[key].Expenses += r.amount ?? 0;
  });
 ...
 */
export default function Analytics({ filtered, allIncome, allExpenses, allEvents = {} }: AnalyticsProps) {
  ...
  // Cast Income[]/Expense[] to the expected shape for avgMonthly
  const avg = useMemo(
    () => avgMonthly(
  ...
  const monthlyInterest = filtered.reduce((s: number, p) =>
    (p.loan_amount ?? 0) > 0 && (p.mortgage_rate ?? 0) > 0 ? s + (p.loan_amount ?? 0) * (p.mortgage_rate ?? 0) / 100 / 12 : s, 0);
  ...
  return (
    <div style={{ padding: '1.25rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem', paddingTop: '0.25rem' }}>
        ...
        {cardOER(m.oer as number, m.expOER as number, avgWindow)}

        {cardYtdOpProfit(agg.ytdNetBalance, agg.market as number, 'YTD Operating Profit')}
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.65rem' }}>
        ...
        {cardAvgExpenses(avg.expenses, agg.totalExpectedOpEx > 0 ? agg.totalExpectedOpEx + avg.mortgage : null as unknown as number, avgWindow, agg.totalMonthlyRent)}
        ...
        <div className="chart-container" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.capRates} layout="vertical" margin={{ top: 8, right: 48, left: 0, bottom: 0 }}>
              ...
              <XAxis type="number" stroke="#9ca3af" tick={{ fontSize: 11 }} tickFormatter={(v: number) => `${v}%`} domain={[0, 'auto']} />
              <YAxis type="category" dataKey="name" stroke="#9ca3af" tick={{ fontSize: 11 }} width={70} />
              <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`, 'Cap Rate']} />
              ...
              <Bar dataKey="Cap Rate" radius={[0,4,4,0]}>
                ...
                <LabelList dataKey="Cap Rate" position="right" formatter={(v: number) => `${v}%`} style={{ fill: '#9ca3af', fontSize: 11 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="chart-container" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.roi} margin={{ top: 16, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
              <XAxis dataKey="name" stroke="#9ca3af" tick={{ fontSize: 11 }} />
              <YAxis stroke="#9ca3af" tick={{ fontSize: 11 }} tickFormatter={(v: number) => `${v}%`} />
        ...
        <div className="chart-container" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.equityPct} margin={{ top: 16, right: 8, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
              <XAxis dataKey="name" stroke="#9ca3af" tick={{ fontSize: 11 }} />
              <YAxis stroke="#9ca3af" tick={{ fontSize: 11 }} domain={[0, 100]} tickFormatter={(v: number) => `${v}%`} />
              <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`, 'Equity']} />
              ...
              <Bar dataKey="EquityPct" radius={[4,4,0,0]}>
                ...
                <LabelList dataKey="EquityPct" position="top" formatter={(v: number) => `${v}%`} style={{ fill: '#9ca3af', fontSize: 10 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="chart-container" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.appreciation} margin={{ top: 16, right: 8, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="name" stroke="#9ca3af" tick={{ fontSize: 11 }} />
              <YAxis stroke="#9ca3af" tick={{ fontSize: 11 }} tickFormatter={(v: number) => `$${(v/1000).toFixed(0)}k`} />
              <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number, n: string) => [`$${v.toLocaleString()}`, n]} />
              <ReferenceLine y={0} stroke="#6b7280" strokeDasharray="4 2" />
              <Legend />
        ...
        <div className="chart-container" style={{ margin: 0 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData.ltv} layout="vertical" margin={{ top: 8, right: 48, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" horizontal={false} />
              <XAxis type="number" stroke="#9ca3af" tick={{ fontSize: 11 }} tickFormatter={(v: number) => `${v}%`} domain={[0, 100]} />
              <YAxis type="category" dataKey="name" stroke="#9ca3af" tick={{ fontSize: 11 }} width={70} />
              <Tooltip contentStyle={CHART_TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`, 'LTV']} />
              ...
              <Bar dataKey="LTV" radius={[0,4,4,0]}>
                ...
                <LabelList dataKey="LTV" position="right" formatter={(v: number) => `${v}%`} style={{ fill: '#9ca3af', fontSize: 11 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
