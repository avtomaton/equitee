...
import { fmt, fmtPeriod, NumInput, SliderInput } from './uiHelpers';
...
function ResultRow({ label, value, cls = '', secondary, tooltip }: ResultRowProps) {
  return (
    <MetricCard
      label={label}
      primary={value}
...
interface CalcRenoInput {
  currentPrice: number;
  plannedPrice: number;
  yearlyApprPct: number;
...
function pbDisplay(months: number | null): { val: string; cls: string } {
...
export default function RenovationView() {
  ...
  const setAdj_ = (k: string, v: number) => setAdj(p => ({ ...p, [k]: v }));
                ...
                {[
                  ...
                  return (
                    <tr key={ri} style={{ borderBottom: '1px solid var(--border)' }}>
                      {row.map((cell, ci) => {
                        let content: React.ReactNode = cell as React.ReactNode;
                        ...
                        if (cell && typeof cell === 'object' && 'v' in cell) {
                          ...
                          if (cell.colored) style.color = col;
                        }
