...
// ── ScorePanel — reused display of investment score ───────────────────────────
...
// ── Analysis tips for the evaluator (forward-looking) ────────────────────────

function buildAnalysis({ avgCashFlow, monthlyGain, yearlyAppr, yearlyApprRatio,
  sellingProfit, marketValue, monthlyRent, capRate, cashOnCash,
  ...
  if (!monthlyRent) {
    ...
    items.push({ isPrimary: true, icon: '✅', cls: 'text-success', label: 'Good capital efficiency',
      detail: `Cash-on-cash return of ${fmtP(cashOnCash * 100)} means your down payment works hard. Cap rate ${fmtP(capRate * 100)} is in acceptable range.` });
  } else if (capRate > 0.06) {
    items.push({ isPrimary: true, icon: '📈', cls: 'text-success', label: 'Decent yield',
      detail: `Cap rate of ${fmtP(capRate * 100)} indicates reasonable asset yield. Cash-on-cash ${fmtP(cashOnCash * 100)} — consider increasing down payment to improve capital returns.` });
  } else {
    items.push({ isPrimary: true, icon: '📊', cls: 'text-warning', label: 'Moderate investment',
      detail: `Cap rate ${fmtP(capRate * 100)} and cash-on-cash ${fmtP(cashOnCash * 100)} are below strong-investment thresholds. Review rent pricing or expenses.` });
  }
  ...
  if (monthlyRent > 0 && expenseRatio > 0.50) {
    items.push({ icon: '💸', cls: 'text-danger', label: 'High expense ratio',
      detail: `${fmtP(expenseRatio * 100)} of rent is consumed by expenses. Consider negotiating purchase price, refinancing, or increasing rent to improve margins.` });
  }
...
export default function EvaluatorView() {
  ...
  const [scenario, setScenario] = useState({
    rentDelta:        0,    // % change from base rent
    downPaymentDelta: 0,    // % change from base down payment
    rateDelta:        0,    // percentage points added to mortgage rate
    vacancyRate:      0,    // % of year vacant
    oneOffExpense:    0,    // $ one-time upfront cost (reno, inspection, etc.)
    repairReserve:    1.0,  // % of purchase price per year set aside for repairs
  });
  ...
  const m = useMemo(() => {
    ...
    // Annualised (gross, before vacancy) for ratio metrics
    const annualGrossRent  = effectiveRent * 12;
    const annualNetOpIncome = annualGrossRent - (p.monthlyOpEx + monthlyTax) * 12; // NOI excl. mortgage
    ...
    const equity        = effectiveDownPayment; // at purchase, equity = down payment
    ...
    const analysis = buildAnalysis({
      ...
      monthlyRent: effectiveRent, capRate, cashOnCash,
      expenseRatio, ltvRatio, equity,
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '1.5rem', alignItems: 'start', marginBottom: '1.5rem' }}>
        ...
        <div className="detail-panel" style={{ display: 'flex', flexDirection: 'column', gap: '0' }}>
          ...
          <NumInput label="Down Payment"          value={inputs.downPayment}        onChange={v => set('downPayment', v)}        prefix="$" min={0} step={1000}
            help={inputs.purchasePrice > 0 ? `${fmtP(inputs.downPayment / inputs.purchasePrice * 100)} of purchase price` : undefined} />
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {mc({ label: 'Loan-to-Value', primary: fmtPct(m.ltvRatio),
            ...
            {mc({ label: 'Cap Rate', primary: fmtPct(m.capRate),
            ...
            {mc({ label: 'Cash-on-Cash', primary: fmtPct(m.cashOnCash),
            ...
            {inputs.monthlyRent > 0 && mc({ label: 'Expense Ratio', primary: fmtPct(m.expenseRatio),
            ...
            {inputs.monthlyRent > 0 && mc({ label: 'Rent-to-Value', primary: fmtPct(m.rentToValue),
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {mc({ label: 'Monthly Mortgage', primary: fmt(m.monthlyMortgage) + '/mo', primaryCls: 'text-danger',
              tooltip: `Standard amortization payment at ${fmtP(m.effectiveRate)}% over ${inputs.amortization} years.` })}
      ...
      <div className="detail-panel" style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem 2rem' }}>
          <SliderInput
            label="Rent adjustment" value={scenario.rentDelta}
          ...
          <SliderInput
            label="Down payment adjustment" value={scenario.downPaymentDelta}
          ...
          <SliderInput
            label="Interest rate stress" value={scenario.rateDelta}
            ...
            help={`Effective rate: ${fmtP(m.effectiveRate)}`} />
          <SliderInput
            label="Vacancy rate" value={scenario.vacancyRate}
          ...
          <SliderInput
            label="One-off expense" value={scenario.oneOffExpense}
          ...
          <SliderInput
            label="Annual repair reserve" value={scenario.repairReserve}
      ...
      <div className="detail-panel">
        <div className="detail-panel-title">
          ...
          <span style={{ fontSize: '0.78rem', color: 'var(--text-tertiary)', fontWeight: 400 }}>
            Based on {fmtP(inputs.annualAppreciation)}% appreciation · {fmt(m.avgCashFlow)}/mo cash flow · {fmtP(m.effectiveRate)}% mortgage
          </span>
        </div>
