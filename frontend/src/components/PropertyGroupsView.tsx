...
export default function PropertyGroupsView() {
  ...
  const [editing, setEditing] = useState<null | 'new' | number>(null); // null | 'new' | group.id
  const [editName, setEditName] = useState('');
  const [editPropIds, setEditPropIds] = useState<number[]>([]);
  ...
  const startEdit = (g: Group) => {
    ...
    setEditIsDefault(g.is_default ?? false);
  };

  const cancel = () => setEditing(null);

  const toggleProp = (pid: number) => {
    setEditPropIds(prev =>
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing === 'new') {
        ...
        await editGroup(editing as number, payload);
      }
      setEditing(null);
    } catch (err) {
  ...
  const handleSetDefault = async (g: Group) => {
    try {
      if (g.is_builtin) {
      ...
      alert(err instanceof Error ? err.message : 'Failed to set default group');
    }
  };
