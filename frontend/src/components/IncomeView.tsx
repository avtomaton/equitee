...
import type { Property, Income } from '../types';

const INCOME_TYPE_COLORS: Record<string, string> = {
  Rent: '#10b981', Deposit: '#3b82f6', Parking: '#f59e0b', Laundry: '#8b5cf6', Other: '#888780',
};
const incomeColorFor = (t: string) => INCOME_TYPE_COLORS[t] ?? '#888780';

function IncomePieChart({ filtered }: { filtered: Record<string, unknown>[] }) {
  ...
  const slices = useMemo(() => {
    ...
    for (const r of filtered) { const t = r.income_type as string; totals[t] = (totals[t] || 0) + (r.amount as number); }
  ...
  useEffect(() => {
    ...
    chartRef.current = new Chart(canvasRef.current, {
      type: 'doughnut',
      data: { labels: slices.map(x => x.t), datasets: [{ data: slices.map(x => x.amount), backgroundColor: slices.map(x => incomeColorFor(x.t)), borderColor: 'rgba(0,0,0,0)', borderWidth: 0, hoverOffset: 6 }] },
      options: { cutout: '62%', animation: { duration: 300 }, plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx: { parsed: number }) => ` $${Math.round(ctx.parsed).toLocaleString()} (${total > 0 ? (ctx.parsed / total * 100).toFixed(1) : 0}%)` } } } },
    });
  ...
  return (
    <div style={{ display: 'flex', gap: '2.5rem', alignItems: 'center', flexWrap: 'wrap', padding: '1rem 0' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', flex: 1, minWidth: 160 }}>
        {slices.map(({ t, amount }: { t: string; amount: number }) => {
...
export default function IncomeView({ properties, onAddIncome, onEditIncome, initialPropertyId, onRegisterReload }: {
  properties: Property[];
  onAddIncome: () => void;
  onEditIncome: (income: Record<string, unknown>) => void;
  initialPropertyId?: number | null;
  onRegisterReload?: ((fn: (() => Promise<void>) | null) => void) | null;
}) {
  const tx = useTransactionView({
    viewName: 'income', endpoint: 'income', dateField: 'income_date', defaultSortBy: 'income_date',
    properties, initialPropertyId: initialPropertyId ?? null,
  ...
  useEffect(() => {
    if (onRegisterReload) { onRegisterReload(() => loadRef.current()); return () => { onRegisterReload(null); }; }
  }, [onRegisterReload]);
  ...
  return (
    <>
      <div className="page-header">
        <div><h1 className="page-title">Income</h1><p className="page-subtitle">Track property income and revenue</p></div>
        <div className="page-actions"><button className="btn btn-primary" onClick={onAddIncome}>+ Add Income</button></div>
      </div>

      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))' }}>
        <StatCard label={propName ? `Total Income — ${propName}` : 'Total Income'} value={`$${tx.filteredTotal.toLocaleString()}`} cls="text-success" sub={amountSub ?? undefined} />
        <StatCard label={propName ? `Transactions — ${propName}` : 'Transactions'} value={tx.filtered.length} sub={txSub ?? undefined} />
      </div>
      ...
      <div className="table-container" style={{ marginBottom: '1rem' }}>
        ...
        {breakdownOpen && (
          ...
          : tx.filtered.length === 0 ? <div className="empty-state" style={{ padding: '1.5rem' }}><div className="empty-state-text">No income in this period</div></div>
      ...
      <div className="table-container">
        <div className="table-header">
          ...
          <div className="table-controls">
            ...
            <div className="sort-group">
              ...
              <button className="btn btn-secondary btn-small" onClick={() => tx.setSortOrder(o => o === 'asc' ? 'desc' : 'asc')}>{tx.sortOrder === 'asc' ? '↑' : '↓'}</button>
            </div>
          </div>
        </div>
        ...
        : tx.filtered.length === 0 ? (
          <div className="empty-state"><div className="empty-state-icon">💰</div><div className="empty-state-text">No income found</div></div>
        ) : (
          <div className="table-scroll-wrap">
            <table>
              ...
              <tbody>
                 {(tx.filtered as Income[]).map(i => (
                  <tr key={i.id as number}>
                    {col('date') && <td className="col-shrink">{fmtDate(i.income_date as string)}</td>}
                    {col('property') && <td className="col-fill"><TruncatedCell text={i.property_name as string} /></td>}
                    {col('amount') && <td className="col-shrink text-success">${(i.amount as number).toLocaleString()}</td>}
                    {col('type') && <td className="col-shrink">{i.income_type as string}</td>}
                    {col('notes') && <td className="col-fill"><TruncatedCell text={i.notes as string} /></td>}
                    <td>
