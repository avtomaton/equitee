...
interface EvalResult {
  ...
  monthlyGain: number;
  cashInvested: number;
  payback: { primary: string; primaryCls: string };
  breakEven: { primary: string; primaryCls: string };
  projections: Array<{ year: number; propertyValue: number; balance: number; yearEquity: number; cashPL: number; totalGain: number }>;
  score: { score: number; starsData: { full: number; half: boolean; empty: number }; label: string; cls: string };
  analysis: Array<{ isPrimary?: boolean; icon: string; cls: string; label: string; detail: string }>;
  grm: number | null;
  irr10: number | null;
  dscr: number | null;
  debtYield: number | null;
  maxVacancy: number | null;
  maxVacancyForGain: number | null;
  minRent: number;
  minRentForGain: number;
  equityMultiple: number | null;
}

function computeEval(inputs: EvalInputs, scenario: EvalScenario = DEFAULT_SCENARIO): EvalResult {
  const p = inputs;
  const s = scenario;
...
function ScorePanel({ score }: { score: { score: number; starsData: { full: number; half: boolean; empty: number }; label: string; cls: string } }) {
  return (
    <div style={{
...
function buildAnalysis({ avgCashFlow, monthlyGain, yearlyAppr, yearlyApprRatio,
  ...
  oneOffExpense, repairReserve }: {
    avgCashFlow: number; monthlyGain: number; yearlyAppr: number; yearlyApprRatio: number;
    cardMarketValue: number; monthlyRent: number; capRate: number; cashOnCash: number;
    expenseRatio: number; ltvRatio: number; equity: number; vacancyRate: number;
    oneOffExpense: number; repairReserve: number;
  }) {
...
function MonthlyCashFlowPanel({ m, vacancyRate }: { m: EvalResult; vacancyRate: number }) {
  ...
  const fv = (n: number) => fmt(Math.abs(n) * mul);
  const sgn = (n: number) => n < 0 ? '−' : '+';

  const Row = ({ label, value, cls = '', indent = 0, bold = false, dim = false, borderTop = false }: {
    label: string; value: string | ReactNode; cls?: string; indent?: number; bold?: boolean; dim?: boolean; borderTop?: boolean;
  }) => (
    <div style={{
  ...
  const ResultBlock = ({ title, sub, value, cls, bg }: { title: string; sub: string; value: string; cls: string; bg: string }) => (
...
function ProjectionTable({ projections, downPayment }: { projections: EvalResult['projections']; downPayment: number }) {
  return (
...
function CompareInputColumn({ col, idx, onChange, onRemove, canRemove, m, onOpenSingle }: {
  col: EvalInputs; idx: number; onChange: (col: EvalInputs) => void; onRemove: () => void; canRemove: boolean; m: EvalResult; onOpenSingle: () => void;
}) {
  const set = (key: keyof EvalInputs, val: number | string) => onChange({ ...col, [key]: val });
...
function CompareRow({ label, values, fmtFn, highlight = 'high', indent = false }: {
  label: string; values: (number | null)[]; fmtFn: (v: number) => string; highlight?: string; indent?: boolean;
}) {
...
function CompareSectionHeader({ label, colCount }: { label: string; colCount: number }) {
  return (
...
function CompareTable({ cols, metrics }: { cols: EvalInputs[]; metrics: EvalResult[] }) {
  const v = (fn: (m: EvalResult) => number | null) => metrics.map(m => m ? fn(m) : null);
  const n = cols.length;
...
export default function EvaluatorView() {
  ...
  const set  = (key: keyof EvalInputs, val: number | string) => setInputs(s  => ({ ...s, [key]: val }));
  const setS = (key: keyof EvalScenario, val: number) => setScenario(s => ({ ...s, [key]: val }));

  const [refi, setRefi] = useState({ currentLoan: 400000, currentRate: 5.5, newRate: 4.5, newAmort: 25, closingCosts: 3000 });
  const setR = (key: string, val: number) => setRefi(s => ({ ...s, [key]: val }));
  ...
  const removeCompareCol = (i: number) => setCompareCols(prev => prev.filter((_, idx: number) => idx !== i));
  const updateCompareCol = (i: number, col: EvalInputs) => setCompareCols(prev => prev.map((c: EvalInputs, idx: number) => idx === i ? col : c));
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '1.5rem', alignItems: 'start', marginBottom: '1.5rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
              ...
              {cardEvalCashOnCash(m.cashOnCash ?? 0)}
              {inputs.monthlyRent > 0 && cardEvalExpenseRatio(m.expenseRatio)}
              {m.equityMultiple !== null && cardEvalEquityMultiple(m.equityMultiple)}
            </div>
            <p className="stat-section-label">Returns &amp; Recovery</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
              {cardEvalAnnualNOI(m.annualNetOpIncome)}
              {cardEvalPayback(m.cashInvested > 0 && m.avgCashFlow > 0 ? m.cashInvested / m.avgCashFlow : null)}
              {cardEvalBreakEven(m.monthlyGain > 0 && m.cashInvested > 0 ? -m.cashInvested / m.monthlyGain : null)}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
              ...
              {inputs.monthlyRent > 0 && cardEvalMaxVacancy(m.maxVacancy, m.avgCashFlow)}
              {inputs.monthlyRent > 0 && cardEvalMinRent(m.minRent)}
            </div>
