 ...
 */
export const mergeOptions = (seed: string[], dataValues: string[]): string[] =>
  [...new Set([...seed, ...dataValues.filter(Boolean)])];

// ── Date helpers ──────────────────────────────────────────────────────────────

/** Parse a YYYY-MM-DD string as a local date (avoids UTC midnight offset issues). */
export const parseLocalDate = (str: string | null): Date | null => {
  if (!str) return null;
...
export const isDateInRange = (date: string | null, start: Date, end: Date): boolean => {
...
export const isCurrentTenant = (tenant: { lease_start?: string; lease_end?: string }): boolean => {
 ...
 *   ≥12 →  '1.2 yr'
 */
export const fmtPeriod = (months: number): string => {
 ...
 */
export const makeInTrailingYear = (): ((dateStr: string | null) => boolean) => {
