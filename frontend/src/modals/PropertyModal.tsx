...
import { ModalOverlay, DateInput, selectOnFocus } from './ModalBase';
...
// Type definitions - supports both camelCase (form) and snake_case (API)
...
interface PropertyModalProps {
  property: import('../types').Property | null;
  onClose: () => void;
  onSave: () => void;
...
const toFormState_Property = (p: import('../types').Property | null): PropertyFormData => p ? {
  ...
  postalCode:           (p as any).postal_code          ?? (p as any).postalCode          ?? '',
  parking:              (p as any).parking              ?? '',
  purchasePrice:        (p as any).purchase_price       ?? (p as any).purchasePrice       ?? 0,
  marketPrice:          (p as any).market_price         ?? (p as any).marketPrice         ?? 0,
  loanAmount:           (p as any).loan_amount          ?? (p as any).loanAmount          ?? 0,
  mortgageRate:         (p as any).mortgage_rate        ?? (p as any).mortgageRate        ?? 0,
  monthlyRent:          (p as any).monthly_rent         ?? (p as any).monthlyRent         ?? 0,
  possDate:             (p as any).poss_date            ?? (p as any).possDate            ?? '',
  status:               (p as any).status               ?? 'Rented',
  type:                 (p as any).type                 ?? 'Condo',
  notes:                (p as any).notes                ?? '',
  expectedCondoFees:    (p as any).expected_condo_fees        ?? (p as any).expectedCondoFees      ?? 0,
  expectedInsurance:    (p as any).expected_insurance         ?? (p as any).expectedInsurance      ?? 0,
  expectedUtilities:    (p as any).expected_utilities         ?? (p as any).expectedUtilities      ?? 0,
  expectedMiscExpenses: (p as any).expected_misc_expenses     ?? (p as any).expectedMiscExpenses   ?? 0,
  expectedAppreciationPct: (p as any).expected_appreciation_pct ?? (p as any).expectedAppreciationPct ?? 0,
  annualPropertyTax:    (p as any).annual_property_tax      ?? (p as any).annualPropertyTax    ?? 0,
  mortgagePayment:      (p as any).mortgage_payment         ?? (p as any).mortgagePayment      ?? 0,
  mortgageFrequency:    (p as any).mortgage_frequency       ?? (p as any).mortgageFrequency    ?? 'monthly',
} : {
...
export default function PropertyModal({ property, onClose, onSave, onError }: PropertyModalProps) {
  const [formData, setFormData] = useState<PropertyFormData>(() => toFormState_Property(property));
  ...
  const handleStatusChange = (newStatus: string) => {
    ...
    if (errors.status) setErrors(e => { const n = { ...e }; delete n.status; return n; });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (property) {
        await editProperty(property.id, formData as unknown as import('../types').Property);
      } else {
        await addProperty(formData as unknown as import('../types').Property);
      }
      onSave();
    } catch (err) {
      console.error(err);
